...
export default function EditShippingMethodPage({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    Promise.all([fetchMethod(), fetchZones()])
  }, [params.id])
    ...
    try {
      ...
      if (data.success) {
        ...
        setFormData({
          ...
          enabled: data.method.enabled,
          costType: data.method.costType,
          cost: data.method.cost,
          minOrderAmount: data.method.minOrderAmount || undefined,
          estimatedDays: data.method.estimatedDays || '',
          taxable: data.method.taxable,
        })

        if (data.method.weightRanges) {
          setWeightRanges(data.method.weightRanges)
        }

        if (data.method.priceRanges) {
          setPriceRanges(data.method.priceRanges)
        }
      } else {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Modifier la Méthode de Livraison</h1>
          <p className="text-gray-500 dark:text-gray-400 mt-1">{method?.name}</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="bg-white/80 dark:bg-dark-900/50 backdrop-blur-sm rounded-lg p-6 border border-gray-200 dark:border-dark-700/50 shadow-lg shadow-black/10 dark:shadow-black/20">
          ...
          <div className="space-y-4">
            {/* Current Zone (Read-only) */}
            <div>
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                Zone de livraison
              </label>
              <div className="bg-gray-100 dark:bg-dark-800 text-gray-500 dark:text-gray-400 px-4 py-2 rounded-lg">
                {method?.zone.name}
              </div>
              <p className="text-xs text-gray-500 mt-1">
                La zone ne peut pas être modifiée. Créez une nouvelle méthode pour une autre zone.
              </p>
            </div>

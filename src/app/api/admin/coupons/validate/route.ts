...
// POST /api/admin/coupons/validate - Validate coupon code for admin order creation
export async function POST(req: NextRequest) {
  try {
    ...
    const { code, userId, cartItems = [] } = body
    ...
    if (!userId) {
      return NextResponse.json({ error: 'Client requis pour valider le coupon' }, { status: 400 })
    }
    ...
    if (coupon.usageLimitPerUser && coupon.usages.length >= coupon.usageLimitPerUser) {
      validationErrors.push('Ce client a atteint la limite d\'utilisation de ce coupon')
    }
    ...
    if (cartItems.length > 0) {
      ...
      if (hasExcludedProduct) {
        validationErrors.push('Certains produits ne sont pas éligibles pour ce coupon')
      }
      ...
      if (
        ...
        validationErrors.push('Aucun produit n\'est éligible pour ce coupon')
      }
    }
